...
void AsmMachine::AddSymbol(const std::string& name, Value* value) {
    if (value->kind() == Value::kValueKindVar) {
      ...
      } else if (value->type() == Value::kValueTypeFloat){ //the group geovanne
        float* mem = reinterpret_cast<float*>(data_memory_ + static_data_end_addr_);
        FloatValue* float_value = static_cast<FloatValue*>(value);
        *mem = float_value->value();
        addr = static_data_end_addr_ += sizeof(float);
